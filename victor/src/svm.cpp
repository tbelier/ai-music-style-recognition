...
int main()
{
    ...
    std::vector<double> descriptor(N_FEATURE);
    ...
    for (const auto& entry : fs::directory_iterator(folder_path))
    {
        if (entry.is_regular_file())
        {
            ...
            int k_predicted;
            std::vector<double> decision_function(N_CLASS, 0.);
            double best_score = -std::numeric_limits<double>::infinity();
            
            // decision_function = coefficients @ descriptor + intercept
            for (int k_class=0; k_class < N_CLASS; k_class++)
            {
                for (int k_feature=0; k_feature < N_FEATURE; k_feature++)
                    decision_function[k_class] += svm_coefficient[k_class][k_feature] * descriptor[k_feature];
                decision_function[k_class] += svm_intercept[k_class];

                if (decision_function[k_class] > best_score)
                {
                    best_score = decision_function[k_class];
                    k_predicted = k_class;
                }
            }
